...
// 섹션 본문을 독립된 줄의 '---'(3개 이상) 구분선 기준으로 나눈다(표시 전용).
// rofan 제작자가 char_persona에 태묵/백운해/유저 설정을 --- 로 구분해 적는 관례 대응 —
// 데이터·프롬프트는 그대로 두고 카드에서 볼 때만 조각으로 나눠 보여준다.
// 구분선이 없으면 원문 하나만 담은 배열을 반환(기존과 동일 렌더).
export function splitByRule(body: string): string[] {
