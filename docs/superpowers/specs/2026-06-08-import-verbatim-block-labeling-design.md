...
## 향후 확장 (이번 범위 아님 — YAGNI)

실사용에서 **오분류**(내용은 보존되지만 엉뚱한 필드/캐릭터에 배치됨)가 자주 거슬릴 경우에만
검토할 후보. 이번 설계는 AI가 본문을 생성/재서술하지 않으므로 변형·잘림은 이미 제거되고,
잔여 문제는 위치 어긋남(저위험)뿐이라 우선은 넣지 않는다.

- **저장 전 확인 모달**: 가져오기를 2단계로 분리 — ①미리보기(캡처→분류→조립까지, 저장 안 함)
  결과를 반환 → 모달에서 사용자가 확인/수정/선택 → ②확정 요청에서 확인된 것만 저장.
  상태 전달은 클라이언트가 조립 결과 JSON을 들고 있다가 확정 시 되돌려보내는 방식이 가장 단순.
- **재분류 버튼**: 분류만 다시 돌리는 경량 옵션. 단, 본문이 verbatim 고정이라 재실행해도
  결과가 비슷할 수 있어 효용은 제한적. 현재도 삭제 후 재가져오기로 대체 가능.

판단 기준: 위 둘 중 무엇을(혹은 둘 다) 넣을지는 실사용 오분류 빈도를 보고 결정.
