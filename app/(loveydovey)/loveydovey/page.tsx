...
export default function LoveydoveyListPage() {
  ...
  const [chars, setChars] = useState<LChar[]>([])
  ...
  useEffect(() => {
    setEditMode(localStorage.getItem('lovey_edit') === '1')
    setSort((localStorage.getItem('lovey_sort') as SortOption) || 'latest')
    setView((sessionStorage.getItem('lovey_view') as typeof view) || 'active')
    fetchData()
    api.get('/api/center-tags').then(setTagConfig).catch(() => {})
  }, [])

  const handleSort = (v: SortOption) => {
    setSort(v); localStorage.setItem('lovey_sort', v)
    if (v === 'random') setRandomSeed(Math.floor(Math.random() * 1e9))
  }

  const handleView = (v: typeof view) => {
    setView(v); sessionStorage.setItem('lovey_view', v)
  }
  ...
  const fetchData = async () => {
    ...
    setHasMore(false)
    try {
      const data: LChar[] = await api.get(`/api/collections?isLoveydovey=true&limit=${FETCH_SIZE}`)
      setChars(data)
  ...
  const loadMore = async () => {
    ...
    try {
      const data: LChar[] = await api.get(`/api/collections?isLoveydovey=true&limit=${FETCH_SIZE}&offset=${chars.length}`)
      setChars(prev => [...prev, ...data])
      setHasMore(data.length === FETCH_SIZE)
    } catch {} finally { setFetchingMore(false) }
  }

  const scrollRef = useScrollRestore(`lovey_scroll_${view}`, !loading)
    ...
    <>
      ...
      <div style={{ display: 'flex', gap: 6, padding: '8px 16px', alignItems: 'center', justifyContent: 'space-between' }}>
        <div style={{ display: 'flex', gap: 6 }}>
          <button className="lovey-chip" style={{ cursor: 'pointer', border: 'none', background: view === 'active' ? 'var(--l-accent)' : 'var(--l-surface-2)', color: view === 'active' ? '#fff' : 'var(--l-ink-soft)' }} onClick={() => handleView('active')}>진행 중 <span style={{ opacity: 0.55 }}>{counts.active}</span></button>
          <button className="lovey-chip" style={{ cursor: 'pointer', border: 'none', background: view === 'waiting' ? 'var(--l-accent)' : 'var(--l-surface-2)', color: view === 'waiting' ? '#fff' : 'var(--l-ink-soft)' }} onClick={() => handleView('waiting')}>대기 <span style={{ opacity: 0.55 }}>{counts.waiting}</span></button>
          <button className="lovey-chip" style={{ cursor: 'pointer', border: 'none', background: view === 'completed' ? 'var(--l-accent)' : 'var(--l-surface-2)', color: view === 'completed' ? '#fff' : 'var(--l-ink-soft)' }} onClick={() => handleView('completed')}>완결 <span style={{ opacity: 0.55 }}>{counts.completed}</span></button>
          <button className="lovey-chip" style={{ cursor: 'pointer', border: 'none', background: view === 'favorites' ? 'var(--l-accent)' : 'var(--l-surface-2)', color: view === 'favorites' ? '#fff' : 'var(--l-ink-soft)' }} onClick={() => handleView('favorites')}>★ 즐겨찾기</button>
        </div>
      ...
      {searchOpen && (
        <>
          ...
          <TagFilterBar groups={tagGroups} selected={selectedTags} onToggle={toggleTag} onClear={() => setSelectedTags([])} chipClass="lovey-chip" accentVar="--l-accent" counts={tCounts} storageKey="lovey_tagcollapse" />
        </>
      )}
