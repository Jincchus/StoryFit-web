    ...
    <>
      ...
      <div style={{ display: 'flex', gap: 6, padding: '8px 16px', alignItems: 'center', justifyContent: 'space-between' }}>
        <div style={{ display: 'flex', gap: 6 }}>
          <button className="lovey-chip" style={{ cursor: 'pointer', border: 'none', background: view === 'active' ? 'var(--l-accent)' : 'var(--l-surface-2)', color: view === 'active' ? '#fff' : 'var(--l-ink-soft)' }} onClick={() => handleView('active')}>진행 중</button>
          <button className="lovey-chip" style={{ cursor: 'pointer', border: 'none', background: view === 'waiting' ? 'var(--l-accent)' : 'var(--l-surface-2)', color: view === 'waiting' ? '#fff' : 'var(--l-ink-soft)' }} onClick={() => handleView('waiting')}>대기</button>
          <button className="lovey-chip" style={{ cursor: 'pointer', border: 'none', background: view === 'completed' ? 'var(--l-accent)' : 'var(--l-surface-2)', color: view === 'completed' ? '#fff' : 'var(--l-ink-soft)' }} onClick={() => handleView('completed')}>완결</button>
      ...
      {searchOpen && (
        <>
          ...
          <TagFilterBar groups={tagGroups} selected={selectedTags} onToggle={toggleTag} onClear={() => setSelectedTags([])} chipClass="lovey-chip" accentVar="--l-accent" />
        </>
      )}
