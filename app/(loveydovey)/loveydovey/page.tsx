...
export default function LoveydoveyListPage() {
  ...
  const fetchData = async () => {
    setLoading(true)
    try { setChars(await api.get('/api/collections?isLoveydovey=true')) }
  ...
  const handleImport = async () => {
    ...
    try {
      ...
      setImportUrl(''); setMsg('✓ 가져왔습니다 (메타데이터만 — 설정·도입부는 직접 입력)'); setMenuOpen(false)
      await fetchData()
    ...
    <>
        ...
        {menuOpen && (
          <div className="lovey-menu">
            <div style={{ padding: '10px 10px 4px', display: 'flex', flexDirection: 'column', gap: 4 }}>
              <input className="field" placeholder="https://loveydovey.ai/characters/..." value={importUrl}
      ...
      {searchOpen && (
        <>
          ...
          <TagFilterBar groups={tagGroups} selected={selectedTags} onToggle={toggleTag} onClear={() => setSelectedTags([])} chipClass="lovey-chip" accentVar="--l-accent" counts={tCounts} />
        </>
      )}
