...
// babechat 좋아요 스캔 — 밑작업(미구현 스텁). 세션/쿠키 + 좋아요 API 엔드포인트 확정 후 활성화.
// 활성화 방법:
//   1) lib/likedScan.ts 상단의 어댑터 예시 + 작동 중 라우트(melting/tingle/zeta)를 참고.
//   2) 자격증명 키: babechat_access_token(이미 있음 · Bearer 추정).
...
export async function GET() {
  return NextResponse.json(
    { error: 'babechat 좋아요 스캔은 준비 중입니다. (관리자 쿠키/세션 + 엔드포인트 설정 필요)' },
    { status: 501 },
  )
}
