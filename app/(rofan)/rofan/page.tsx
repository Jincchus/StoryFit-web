...
export default function RofanListPage() {
  ...
  const handleLikedScan = async () => {
    ...
    setScanning(true); setScanMsg('로판 좋아요 목록 스캔 중...')
    try {
      const res = await api.get('/api/rofan/liked-scan')
  ...
  const toggleEditMode = () => {
    ...
    localStorage.setItem('rofan_edit', next ? '1' : '0'); setMenuOpen(false)
  }
    ...
    return (
      <div key={c.id} className="rofan-card" style={{ position: 'relative' }}
        ...
        <div className="rofan-card-body">
          ...
          {c.description?.trim() && (
            <div style={{ fontSize: 11, color: 'var(--r-ink-soft)', lineHeight: 1.4, overflow: 'hidden', textOverflow: 'ellipsis', display: '-webkit-box', WebkitLineClamp: 3, WebkitBoxOrient: 'vertical' }}>
  ...
  return (
    <>
      ...
      />
      <div className="rofan-header" style={{ position: 'relative' }}>
        <div style={{ display: 'flex', alignItems: 'center', gap: 4 }}>
          <button className="rofan-iconbtn" aria-label="홈으로" onClick={() => router.push('/')}>🏠</button>
          <div className="rofan-logo">rofanai</div>
        </div>
        <button className="rofan-iconbtn" onClick={() => setMenuOpen(o => !o)}>⋮</button>
        {menuOpen && (
          <div className="rofan-menu">
            <div style={{ padding: '10px 10px 4px', display: 'flex', flexDirection: 'column', gap: 4 }}>
              ...
              <button className="rofan-menu-item"
                style={{ background: 'var(--r-accent)', borderRadius: 8, color: '#fff', textAlign: 'center' }}
                disabled={importing} onClick={handleImport}>{importing ? '가져오는 중...' : '📥 가져오기'}</button>
            </div>
            <button className="rofan-menu-item" onClick={handleLikedScan}>♥ 좋아요 목록</button>
            <button className="rofan-menu-item" onClick={createCharacter}>+ 새 캐릭터 만들기</button>
            <button className="rofan-menu-item" onClick={toggleEditMode}>
      ...
      <div className="rofan-scroll" ref={scrollRef}>
        {loading ? (
          <div className="rofan-grid">
            {Array.from({ length: 4 }).map((_, i) => (
              <div key={i} className="rofan-card">
          ...
          : view === 'favorites'
            ? <div className="rofan-empty">즐겨찾기한 캐릭터가 없습니다.<br />카드의 ★를 눌러 추가하세요.</div>
          : view === 'completed'
