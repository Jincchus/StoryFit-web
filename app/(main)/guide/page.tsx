...
const FEATURE_SECTIONS: FeatureSection[] = [
  ...
  },
  {
    ...
    items: [
      ...
      { emoji: '✨', label: '제작자 추천 페르소나', desc: '일부 센터(Zeta·멜팅·Tikita)에서 가져온 카드는 제작자가 만들어 둔 추천 페르소나가 함께 등록됩니다. 페르소나 선택 목록 맨 위에 "추천" 배지로 표시되며, 캐릭터 라이브러리에는 섞이지 않고 페르소나로만 쓸 수 있습니다.' },
    ],
  },
  {
    title: '⚙️ 대화 설정',
    items: [
      { emoji: '📂', label: '설정창 4탭 구성', desc: '대화 설정창(⚙)은 기본·AI응답·기억·세계관 4개 탭으로 나뉘고, 맨 위에서 이 대화의 AI 모델(Gemini 2.5/3.1 Pro, 2.5 Flash)을 바로 바꿀 수 있습니다. Pro는 문체·캐릭터 몰입도가 좋고, Flash는 훨씬 빠르고 저렴한 대신 문체가 다소 평이해질 수 있습니다.' },
  ...
  },
  {
    title: '🛠 내 커맨드 & AI 폴백',
    items: [
      { emoji: '⚡', label: '커스텀 AI 커맨드 (내 커맨드)', desc: '대화 설정 > "내 커맨드"에서 "+ 새 커맨드"로 `!에타` 같은 나만의 슬래시 커맨드를 만들 수 있습니다. 저장된 커맨드는 목록에서 펼쳐(아코디언) 내용을 수정하거나 삭제할 수 있고, 모든 대화에서 재사용되며 AI 응답은 마크다운/HTML로 렌더링됩니다. 매번 모양이 달라지지 않게 하려면 지시문에서 앱 공용 클래스(sf-bubble·sf-card 등, 지시문 옆 ? 참고)만 쓰라고 지정하면 스타일이 고정됩니다.' },
  ...
  },
  {
    ...
    items: [
      ...
      { emoji: '↻', label: '다시 가져오기 (빈 칸 채우기)', desc: '이미 가져온 카드를 다시 가져오면(좋아요 목록에서 ✓ 완료 항목을 탭하거나 URL을 다시 붙여넣기) 비어 있던 칸만 원본 값으로 채웁니다. 직접 편집한 내용은 덮어쓰지 않으므로, 원본에 나중에 추가된 비밀설정 등을 안전하게 보충할 때 사용합니다.' },
      ...
      { emoji: '🩶', label: '크랙 국내 센터', desc: 'crack.wrtn.ai의 스토리·캐릭터 URL을 붙여넣으면 스토리/캐릭터 2탭으로 가져옵니다. 스토리로 들어가면 등장 캐릭터 전부를, 캐릭터로 들어가면 그 캐릭터가 나오는 스토리들을 함께 가져오되 더 이상 번지지 않습니다(연쇄 가져오기). 로그인이 필요해 관리자 설정에서 인증 토큰을 먼저 등록해야 합니다.' },
      { emoji: '🏷', label: 'Tikita 상세: AI 전달 / 참고 구분', desc: 'Tikita 스토리 상세 화면은 "대화에 들어가는 설정"(등장인물·소개글/세계관·상세 안내·첫 장면, 각 블록에 "AI 전달" 배지)과 "참고 정보"(일러스트·갤러리·에피소드, 기본 접힘)로 나뉩니다. "AI 전달" 배지가 붙은 내용만 실제 대화 프롬프트에 들어가며, 무거운 세계관 편집은 기본 접힌 상태라 필요할 때만 펼쳐 섹션별로 세계관 포함 여부를 고를 수 있습니다. 가져올 때 패치노트·공지·추천플레이 같은 독자용 잡음 섹션은 자동으로 숨김 처리되어 프롬프트에서 제외됩니다.' },
    ],
  },
  {
    title: '🔍 검색 & 기록',
  ...
  },
  {
    ...
    items: [
      ...
      { emoji: '🧾', label: '장기 메모리', desc: '대화가 길어지면 자동으로 요약되며, 선택해서 핵심 메모리로 승격할 수 있습니다. "☑ 전체 선택"으로 아직 등록하지 않은 항목을 한 번에 골라 올릴 수 있습니다(한 번에 최대 20개). 승격한 항목은 "↩ 해제"로 다시 내려 재선택·재승격할 수 있습니다.' },
