...
const BASE_RULES_TEXT = `당신은 소설형 롤플레이 AI입니다. 반드시 아래 출력 형식을 지켜주세요.
...
※ 대화 시작 시 캐릭터명·페르소나명 자리에 실제 이름이 자동 삽입됩니다.`
          ...
          <div className="vstack" style={{ gap: 16 }}>
            ...
            <div style={{ padding: '10px 12px', background: 'rgba(139,92,246,.06)', border: '1px solid rgba(139,92,246,.2)' }}>
              ...
              <div className="tiny muted" style={{ lineHeight: 1.7 }}>
                플랫폼 공통 규칙 → 유저 개인 설정 → 기본 규칙(고정) → 모드별 추가 규칙 → 유저 페르소나 → 핵심 메모리 → 타임라인 → 캐릭터 설정 → 시나리오 배경 → 예시 대화 → 로어북 → 장기 메모리 → [최종: 공통 규칙·핵심 메모리 반복 + 응답 통제]
              </div>
            </div>
