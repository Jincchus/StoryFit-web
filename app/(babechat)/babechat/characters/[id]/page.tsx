...
export default function BabechatCharDetailPage() {
      ...
      <div style={{ display: 'flex', flexDirection: 'column', flex: 1, minHeight: 0 }}>
        <div className="bc-scroll">
          ...
          <div className="bc-section">
            <div style={{ display: 'flex', gap: 12, alignItems: 'center', marginBottom: 10 }}>
              ...
              <div style={{ minWidth: 0, flex: 1 }}>
                <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
                  <h1 style={{ fontSize: 20, fontWeight: 800, margin: '0 0 4px', color: 'var(--b-ink)', overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' }}>{col.title}</h1>
            ...
            {tagline && <p className="bc-desc" style={{ marginBottom: 10 }}>{replaceDisplayPlaceholders(tagline, userDisplayName, mainChar?.name ?? '')}</p>}
            {col.tags?.length > 0 && (
              <div style={{ display: 'flex', flexWrap: 'wrap', gap: 6 }}>
                {col.tags.map(t => <span key={t} className="bc-chip">#{t}</span>)}
              </div>
            )}
          </div>
          ...
          {mainChar?.additionalInfo?.trim() && (
            <div className="bc-section" style={{ paddingTop: 0 }}>
              <h2 className="bc-section-title">상세 설정</h2>
          ...
          <div className="bc-section" style={{ paddingTop: 0 }}>
            <h2 className="bc-section-title">첫 장면</h2>
            <div style={{ display: 'flex', gap: 6, flexWrap: 'wrap', marginBottom: 10, alignItems: 'center' }}>
              {openings.map((op, i) => (
                <button key={op.id} className={`bc-chip ${i === openingIdx ? 'sel' : ''}`}
                  style={{ border: 'none', cursor: 'pointer' }} onClick={() => { setOpeningIdx(i); setIsEditingOpening(false) }}>
                  {op.title}
                </button>
              ))}
              {!isEditingOpening && (
                <button className="bc-chip" style={{ border: 'none', cursor: 'pointer', background: 'var(--b-surface-2)', marginLeft: 'auto' }}
            ...
            {isEditingOpening ? (
              <div className="vstack" style={{ gap: 8 }}>
                <textarea
                  className="field"
                  style={{ fontSize: 13, background: 'var(--b-surface)', border: '1px solid var(--b-line)', color: 'var(--b-ink)', padding: 10, borderRadius: 10, width: '100%', resize: 'vertical' }}
                  rows={8}
                  value={editContent}
              ...
              <div className="bc-intro-box">
                <NovelText text={replaceDisplayPlaceholders(opening, userDisplayName, mainChar?.name ?? '')} />
              </div>
            ) : (
              <div className="bc-intro-box" style={{ color: 'var(--b-ink-soft)' }}>
                도입부가 없습니다. ✏ 편집으로 첫 장면을 직접 작성하세요.
              </div>
            )}
          </div>
          ...
          {existingConvs.length > 0 && (
            <div className="bc-section" style={{ paddingTop: 0 }}>
              <h2 className="bc-section-title">진행 중인 대화</h2>
              <div style={{ display: 'flex', flexDirection: 'column', gap: 8 }}>
                {existingConvs.map(c => (
                  <div key={c.id} className="bc-card" style={{ cursor: 'pointer', flexDirection: 'row', justifyContent: 'space-between', alignItems: 'center', padding: '10px 14px', background: 'var(--b-surface)', border: '1px solid var(--b-line)', borderRadius: 10 }} onClick={() => router.push(`/conversations/${c.id}`)}>
                    <div style={{ minWidth: 0, flex: 1 }}>
                      <div style={{ fontWeight: 700, fontSize: 13, color: 'var(--b-ink)' }}>{c.title}</div>
                      {c.messages?.[0]?.content && (
                        <div style={{ color: 'var(--b-ink-soft)', fontSize: 12, marginTop: 2, overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' }}>
                          {c.messages[0].content}
                        </div>
                      )}
                    </div>
                    <div style={{ color: 'var(--b-ink-soft)', fontSize: 11, flexShrink: 0, marginLeft: 10 }}>{formatDate(c.updatedAt)}</div>
                  </div>
                ))}
              </div>
            </div>
        ...
        <div className="bc-cta">
          <button className="bc-cta-btn" onClick={handleCtaClick} disabled={!mainChar}>
