...
export default function BabechatListPage() {
  ...
  } = useCenterListPaged({ indexQuery: 'isBabechat=true', storagePrefix: 'bc' })
  ...
  useEffect(() => { setEditMode(localStorage.getItem('bc_edit') === '1') }, [])

  const handleLikedScan = async () => {
    setMenuOpen(false); setLikedPanel(true)
    if (likedList.length > 0) return
    setScanning(true); setScanMsg('babechat 좋아요 목록 스캔 중...')
    try {
      const res = await api.get('/api/babechat/liked-scan')
      setLikedList(res.liked ?? [])
      setScanMsg(`♥ ${res.total ?? (res.liked?.length ?? 0)}개 발견`)
    ...
    return (
      <div key={c.id} className="bc-card" style={{ position: 'relative' }}
        ...
        <div className="bc-card-body">
          ...
          {c.description?.trim() && (
            <div style={{ fontSize: 11, color: 'var(--b-ink-soft)', lineHeight: 1.4, overflow: 'hidden', textOverflow: 'ellipsis', display: '-webkit-box', WebkitLineClamp: 3, WebkitBoxOrient: 'vertical' }}>
    ...
    <>
      ...
      <div className="bc-header" style={{ position: 'relative' }}>
        ...
        {menuOpen && (
          <div className="bc-menu">
            ...
            <button className="bc-menu-item" onClick={handleLikedScan}>♥ 좋아요 목록</button>
      ...
      <div style={{ display: 'flex', gap: 6, padding: '8px 16px', alignItems: 'center', justifyContent: 'space-between' }}>
        <div style={{ display: 'flex', gap: 6 }}>
          <button className="bc-chip" style={{ cursor: 'pointer', border: 'none', background: view === 'active' ? 'var(--b-accent)' : 'var(--b-surface-2)', color: view === 'active' ? '#fff' : 'var(--b-ink-soft)' }} onClick={() => setView('active')}>진행 중 <span style={{ opacity: 0.55 }}>{counts.active}</span></button>
          <button className="bc-chip" style={{ cursor: 'pointer', border: 'none', background: view === 'waiting' ? 'var(--b-accent)' : 'var(--b-surface-2)', color: view === 'waiting' ? '#fff' : 'var(--b-ink-soft)' }} onClick={() => setView('waiting')}>대기 <span style={{ opacity: 0.55 }}>{counts.waiting}</span></button>
          <button className="bc-chip" style={{ cursor: 'pointer', border: 'none', background: view === 'completed' ? 'var(--b-accent)' : 'var(--b-surface-2)', color: view === 'completed' ? '#fff' : 'var(--b-ink-soft)' }} onClick={() => setView('completed')}>완결 <span style={{ opacity: 0.55 }}>{counts.completed}</span></button>
          <button className="bc-chip" style={{ cursor: 'pointer', border: 'none', background: view === 'favorites' ? 'var(--b-accent)' : 'var(--b-surface-2)', color: view === 'favorites' ? '#fff' : 'var(--b-ink-soft)' }} onClick={() => setView('favorites')}>★ 즐겨찾기</button>
        </div>
      ...
      {searchOpen && (
        <>
          ...
          <TagFilterBar groups={tagGroups} selected={selectedTags} onToggle={toggleTag} onClear={clearTags} chipClass="bc-chip" accentVar="--b-accent" counts={tCounts} storageKey="bc_tagcollapse" />
        </>
      )}
      ...
      <div className="bc-scroll" ref={scrollRef}>
        {loading ? (
          ...
          <div className="bc-empty">{error}<br /><button className="bc-chip" style={{ cursor:'pointer', border:'none', background:'var(--b-accent)', color:'#fff', marginTop:8 }} onClick={() => refresh()}>다시 시도</button></div>
