...
export default function BabechatListPage() {
  ...
  const fetchData = async () => {
    setLoading(true)
    try { setChars(await api.get('/api/collections?isBabechat=true')) }
    ...
    <>
        ...
        {menuOpen && (
          <div className="bc-menu">
            <div style={{ padding: '10px 10px 4px', display: 'flex', flexDirection: 'column', gap: 4 }}>
              <input className="field" placeholder="https://babechat.ai/characters/..." value={importUrl}
      ...
      <div style={{ display: 'flex', gap: 6, padding: '8px 16px', alignItems: 'center', justifyContent: 'space-between' }}>
        <div style={{ display: 'flex', gap: 6 }}>
          <button className="bc-chip" style={{ cursor: 'pointer', border: 'none', background: view === 'active' ? 'var(--b-accent)' : 'var(--b-surface-2)', color: view === 'active' ? '#fff' : 'var(--b-ink-soft)' }} onClick={() => handleView('active')}>진행 중</button>
          <button className="bc-chip" style={{ cursor: 'pointer', border: 'none', background: view === 'waiting' ? 'var(--b-accent)' : 'var(--b-surface-2)', color: view === 'waiting' ? '#fff' : 'var(--b-ink-soft)' }} onClick={() => handleView('waiting')}>대기</button>
          <button className="bc-chip" style={{ cursor: 'pointer', border: 'none', background: view === 'completed' ? 'var(--b-accent)' : 'var(--b-surface-2)', color: view === 'completed' ? '#fff' : 'var(--b-ink-soft)' }} onClick={() => handleView('completed')}>완결</button>
      ...
      {searchOpen && (
        <>
          ...
          <TagFilterBar groups={tagGroups} selected={selectedTags} onToggle={toggleTag} onClear={() => setSelectedTags([])} chipClass="bc-chip" accentVar="--b-accent" />
        </>
      )}
